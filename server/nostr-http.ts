...
export async function publishHighFiveToNostr(highFive: {
  ...
  try {
    ...
    // Add the Lightning payment information directly to the post
    if (highFive.lightningInvoice) {
      try {
        // Instead of trying to embed an image, we'll directly add the Lightning 
        // payment information as a tag and in the content
        // This way, Nostr clients that support Lightning can detect and handle it

        // Add Lightning invoice as a 'bolt11' tag for Nostr clients that support it
        event.tags.push(['bolt11', highFive.lightningInvoice]);
        
        // Also add a 'lightning' tag with the invoice for maximum compatibility
        event.tags.push(['lightning', highFive.lightningInvoice]);
        
        console.log('Added Lightning payment information as Nostr tags');
        
      } catch (err) {
        console.error('Error adding Lightning payment info to Nostr post:', err);
      }
    }
